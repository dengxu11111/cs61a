...
def merge(lst1, lst2):
    ...
    lst2_count =0
    x=[]
    while(lst1_count<len(lst1) or lst2_count<len(lst2)):
        if(lst1[lst1_count]<lst2[lst2_count]):
            x=x+[lst1[lst1_count]]
            lst1_count+=1
        else:
            x=x+[lst2[lst2_count]]
    ...
    if lst1_count==len(lst1):
        x=x+lst2[lst2_count:len(lst2)]
    if lst2_count==len(lst2):
        x=x+lst1[lst1_count:len(lst1)]
    return x"""
...
def create_row(size):
    ...
    """
    "*** YOUR CODE HERE ***"
    return ['-' for i in range(size) if i < size]
...
def create_board(rows, columns):
    ...
    """
    "*** YOUR CODE HERE ***"
    return [create_row(columns) for i in range(rows) if i<rows]  
