...
def replace_leaf(t, old, new):
    ...
    """
    "*** YOUR CODE HERE ***"
    bra=[replace_leaf(branch,old,new) for branch in branches(t)]
...
def quadratic(x, a, b, c):
    ...
    low=lower_bound(x)
    up=upper_bound(x)
    low_sum=a*low*low+b*low+c
    up_sum=a*up*up+b*up+c
    temp_sum=a*temp*temp+b*temp+c
    if temp<=low or temp>=up:
        return interval(min(low_sum,up_sum),max(low_sum,up_sum))
    else:
        return interval(min(low_sum,up_sum,temp_sum),max(low_sum,up_sum,temp_sum))
