...
def do_or_form(expressions, env):
    ...
    if expressions==nil:
        ...
        first_expr=scheme_eval(expressions.first,env)
        if not scheme_falsep(first_expr):
            return first_expr
        else:
...
def make_let_frame(bindings, env):
    ...
    new_env=env.make_child_frame(bindings.first.first,scheme_eval(bindings.first.second))
    while bindings.second is not nil:
        bindings=bindings.second
        new_env.bindings[bindings.first.first]=scheme_eval(bindings.first.second)
    return new_env"""
